...
class FormController extends ControllerBase {
  ...
  public static function extractMollomValues(FormState $form_state, $fields, $mapping) {
    ...
    foreach ($fields as $field) {
      ...
      if (isset($exclude_fields[$field])) {
        $mapping[$exclude_fields[$field]] = MollomUtilities::flattenFormValue($value);
    ...
    if (isset($data['authorId'])) {
      ...
      if (!empty($account->getCreatedTime)) {
    ...
    if ($invalid_utf8 || $invalid_xml) {
      ...
      $message = SafeMarkup::format('Invalid !type in form values: %teaser', array('!type' => $invalid_utf8 ? 'UTF-8' : 'XML characters'));
    ...
    if ($entity_info instanceof FieldableEntityInterface) {
      ...
      foreach ($field_instances as $field_name => $field) {
        if (in_array($field->getType(), array(
          ...
          $form_info['elements'][$field_name] = \Drupal\Component\Utility\SafeMarkup::checkPlain(t($field->getLabel()));
        }
      }
    }
  }
  ...
  public static function validateCaptcha(&$form, FormState $form_state) {
    ...
    if ($mollom_form['mode'] == \Drupal\mollom\Entity\FormInterface::MOLLOM_MODE_ANALYSIS) {
      ...
      if (empty($mollom_form['captchaId'])) {
        $form_state->set('mollom_captcha_required', $mollom_form['captcha_type']);
    ...
    // $form_state custom mollom_passed_captcha may only ever be set by this
    ...
    // first place.
    if ($form_state->get('mollom_passed_captcha')) {
    ...
    if (!empty($result['solved'])) {
      ...
      if ($require_analysis) {
        $form_state->setValue(array('mollom', 'captchaId'), '');
        $form['mollom']['captchaId']['#value'] = '';
      }
      $form_state->set('mollom_passed_captcha', TRUE);
      $form['mollom']['captcha']['#access'] = FALSE;
      $form['mollom']['captcha']['#solved'] = TRUE;
  ...
  public static function validateAnalysis(&$form, FormState $form_state) {
    ...
    if ($mollom['mode'] != \Drupal\mollom\Entity\FormInterface::MOLLOM_MODE_ANALYSIS) {
      return FALSE;
    }
    ...
    if (is_array($mollom['checks'])) {
      $checks_to_add = array();
      foreach ($mollom['checks'] as $check_id => $check) {
        if ($check_id == $check) {
          $checks_to_add[] = $check;
        }
      }
    ...
    $form_state->setValue(array('mollom' => 'require_captcha'), FALSE);
    $form['mollom']['captcha']['#access'] = FALSE;
      ...
      switch ($result['spamClassification']) {
        ...
        case 'unsure':
          if ($mollom['unsure'] == 'moderate') {
            ...
            $form_state->set('mollom_passed_captcha', FALSE);
            ...
            $form_state->setCached(FALSE);
            // Set the CAPTCHA type required indicator as well as directly in
            // the form values.
            $form_state->set('mollom_captcha_required', $mollom['captcha_type']);
  ...
  public static function submitForm($form, FormState $form_state) {
    ...
    if (empty($mollom) || empty($mollom['entity'])) {
      return;
    }
