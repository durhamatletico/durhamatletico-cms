...
class MollomUtilities {
  ...
  public static function displayMollomTestModeWarning() {
    ...
    $omit_warning = \Drupal::state()->get('mollom.omit_warning') ?: FALSE;
    $warned = &drupal_static(__FUNCTION__, $omit_warning);
    if ($warned) {
      return;
    }
    $warned = TRUE;

    if (\Drupal::config('mollom.settings')
        ->get('test_mode.enabled') && empty($_POST)
    ) {
      $admin_message = '';
