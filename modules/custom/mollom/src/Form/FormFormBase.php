...
class FormFormBase extends EntityForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($entity->isNew() && !empty($input['id'])) {
      ...
      foreach ($entity->enabled_fields as $value) {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      ...
      drupal_set_message($this->t('Mollom Form %label has been updated.', array('%label' => $entity_label)));
      \Drupal::logger('mollom')->notice('Mollom Form %label has been updated.', array('%label' => $entity_label));
    }
    else {
      // If we created a new entity...
      drupal_set_message($this->t('Mollom form %label has been added.', array('%label' => $entity_label)));
